...
Exchange.prototype.publish = function (routingKey, data, options, callback) {
  ...
  var avroSchema;
  if(options.contentType.indexOf('avro/json') > -1)
  {
    avroSchema = options.contentType.substring(10, options.contentType.length);
    if(typeof(options.schema) === 'string' && avroSchema != null && 0 !== options.schema.length)
    {
      var type = rova.parse(`${options.schema}/${avroSchema}.avsc`);
      try{
        type.validate(data);
      }
      catch(e)
