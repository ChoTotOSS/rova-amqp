...
Queue.prototype.subscribe = function (options, messageListener) {
  ...
  return this.subscribeRaw(rawOptions, function (m) {
    ...
    var avroSchema;
    if(contentType.indexOf('avro/json') > -1)
    {
      avroSchema = contentType.substring(10, contentType.length);
    }
    ...
    m.addListener('end', function () {
      ...
      if(typeof(rawOptions.schema) === 'string' && avroSchema != null && 0 !== rawOptions.schema.length)
      {
        var type = rova.parse(`${rawOptions.schema}/${avroSchema}.avsc`);
        type.validate(json);
      }
